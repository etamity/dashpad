...
const watchFileJsx = ['*.js', '*.jsx', '*.mdx', '*.css'];

const watchRoot = process.env.APP_PWD
    ? path.join(process.env.APP_PWD, '_dash')
    : pathHelper.PACKAGES;
const watchFiles = [...watchFileYaml, ...watchFileJsx].map(ext =>
    path.join(watchRoot, '/**/', ext)
);
...
watcher.on('ready', () => {
    log.info('Start Watching ...', allWatchFiles);
